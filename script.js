function Qismo(appID, options){
    ...
    function initiateQismo() {
        ...
        $ajax({
            type: 'GET',
            url: defaultConfig.qismoBaseUrl+'/api/v1/app/config/public-widget/'+appID,
    ...
    function attachQismoCustomCSS(style) {
        if(!style) {
            $('head').appendChild(createNode('style', '.qcw-header { background: #74c162 !important; }'));
            return false;
        }
        ...
        $('head').appendChild(createNode('style', cssstring));
    }
    ...
    function renderQismoWidget() {
        // attach our container first
        $('body').insertAdjacentHTML('afterbegin', '<div class="qismo-extra"></div>');
        // now let's decide whether we should render login form or chat widget directly
        if(!ls || (!ls.user_id && !ls.user_name)) {
            renderQismoWidgetComponent();
        } else {
            initQiscusWidget(ls);
        }
    }
    ...
    function attachQiscusSDKDependencies() {
        ...
        sdkjs.src = 'https://qiscus-sdk.s3-ap-southeast-1.amazonaws.com/public/qiscus-sdk.2.8.5-rc3.js';
        ...
        var sdkcss = createNode('link', null, {
            'rel': 'stylesheet',
            'href': 'https://d1edrlpyc25xu0.cloudfront.net/kiwari-prod/raw/upload/_hHKEkgr4H/qiscus-sdk.2.8.5-rc1.css',
        });
        var qismocss = createNode('link', null, {
            'rel': 'stylesheet',
            'href': 'https://s3-ap-southeast-1.amazonaws.com/qiscus-sdk/public/qismo/qismo-latest.css',
            // 'href': '/qismo-latest.css',
        });

        $('head').appendChild(sdkcss);
        $('head').appendChild(qismocss);
        $('body').insertAdjacentHTML('afterbegin', '<div id="qiscus-widget"></div>');
    }

    function renderQismoTriggerButton() {
        var buttonText = (qismoConfig.buttonHasText)
                        ? ((qismoConfig.buttonText) ? qismoConfig.buttonText : defaultConfig.buttonText)
                        : '',
            buttonIcon = qismoConfig.buttonIcon || defaultConfig.buttonIcon,
            // buttonContent = '<img src="'+buttonIcon+'" class="qismo-trigger__icon"><div class="qismo-triger__text">'+buttonText+'</div>',
            imagePart = qismoConfig.buttonIcon ? '<img src="'+buttonIcon+'">' : '',
            textPart = buttonText ? '<div>'+buttonText+'</div>' : '',
            buttonContent = imagePart + textPart;
            button = createNode('div', null, {
                'class': 'qcw-trigger-btn qcw-cs-trigger-button'
            });
            button.insertAdjacentHTML('afterbegin', buttonContent);
        $('.qcw-cs-container').appendChild(button);
        // let's also attach its' event listener
        // addEvent(button, 'click', function(){
        //     loginFormOpened = !loginFormOpened;
        //     if($('.qcw-cs-welcome')) $('.qcw-cs-welcome').style.display = 'none';
    ...
    function renderQismoWidgetComponent() {
        if(qismoConfig.attentionGrabberStatus) attachAttentionGrabber();
        if(qismoConfig.welcomeMessageStatus && !qismoConfig.attentionGrabberStatus) attachWelcomeDialog();
    ...
    function attachWelcomeDialog(){
        ...
        if(qismoConfig.welcomeMessageStatus && welcomeDialogStatus){
            ...
            setTimeout(function(){
                if($('.qcw-cs-welcome')) return false;
                if(!loginFormOpened) {
                    $('.qismo-extra').insertAdjacentHTML('afterbegin', welcomeContainer);
                }
    ...
    function attachEventListenerChat(){
        ...
        if (screen.width > 768) {
            document.addEventListener('click', function(e) {
                ...
                if(filterClass(e)) {
                    closeWelcomeDialog();
                    if ($('.qcw-cs-container--open') || $('.qcw-container--open')) {
                        loginFormOpened = false;
                        if(qismoConfig.buttonIcon) $('.qcw-trigger-btn img').src=qismoConfig.buttonIcon;
                        fadeOut($('.qcw-cs-box-form'), function(){
                            $('.qcw-cs-container').classList.remove('qcw-cs-container--open')
                        });
                    } else {
                        loginFormOpened = true;
                        if(qismoConfig.buttonIcon) $('.qcw-trigger-btn img').src=closeIcon;
                        fadeIn($('.qcw-cs-box-form'));
                        if($('.qcw-cs-container')) return $('.qcw-cs-container').classList.add('qcw-cs-container--open')
                    }
                }
            }, false)
        } else {
            document.addEventListener('click', function(e) {
                ...
                if(filterClass(e)) {
                    ...
                    if ($('.qcw-cs-container--open') || $('.qcw-container--open')) {
                        ...
                        loginFormOpened = true;
                        $('.qcw-cs-container').classList.add('qcw-cs-container--open')
    ...
    /** Attention Grabber */
    function attachAttentionGrabber() {
        ...
        if(qismoConfig.attentionGrabberStatus){
            // render the layout
            $('.qismo-extra').insertAdjacentHTML('beforeend', '<div class="qismo-attention-grabber">' +
                '<span class="qismo-attention-grabber__close-btn">&times;</span>' +
            '</div>');
            if(grabberImage) {
                $('.qismo-extra .qismo-attention-grabber').insertAdjacentHTML('afterbegin', '<div class="qcw-grabber-image">'+
                    '<img src="'+attentionGrabberImage+'" />' +
                '</div>');
            }
            if(grabberTextStatus) {
                $('.qismo-extra .qismo-attention-grabber').insertAdjacentHTML('beforeend', '<div class="qcw-grabber-text">'+
                    '<div>'+ attentionGrabberText +'</div>' +
                '</div>');
            }
            // attach the listeners
            addEvent($('.qismo-attention-grabber__close-btn'), 'click', function() {
                $('.qismo-attention-grabber').style.display = 'none';
            })
        }
    }
    ...
    function ValidateEmail(mail) {
        if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test($('#inputEmail').value)) return true;
        toggleClass($('#inputEmail').parentElement, 'qismo-input-error');
        toggleClass($('.qismo-form-error.qismo-email-error'), 'qismo-form-error--visible');
        return false;
    }
    ...
    function attachLoginFormToDOM() {
        ...
        var chatForm = '<div class="qcw-cs-container">' +
            ...
            '</div>' +
        '</div>';
        $('body').insertAdjacentHTML('afterbegin', chatForm);
        renderQismoTriggerButton();
        ...
        addEvent($('.qcw-cs-wrapper form'), 'submit', function (e) {
            e.preventDefault();
            var _self = $('.qcw-cs-wrapper form'),
                submitBtn = $('button[name="submitform"]'),
                randomKey = Date.now(),
                userData = {
                    user_id: $('#inputEmail').value.toLowerCase(),
                    user_name: $('#inputname').value
                }

            toggleClass($('.qismo-input-error'), 'qismo-input-error');
            toggleClass($('.qismo-form-error--visible'), 'qismo-form-error--visible');

            if(!ValidateEmail()) return false;
            if (!userData.user_name) {
                toggleClass($('#inputname').parentElement, 'qismo-input-error');
                return toggleClass($('.qismo-form-error.qismo-name-error'), 'qismo-form-error--visible');
            }

            // callback, if you want add extra action when user hit submit button at form welcome
            var afterFormValidation = defaultConfig.callbackFunction.afterFormValidation;
    ...
    const defaultInitOptions = {
        ...
        roomChangedCallback: function (data) {
            ...
            if (!$('.qcw-copyright')) {
                ...
                '</div>';
                if($('.qcw-chat-wrapper')) $('body .qcw-chat-wrapper').insertAdjacentHTML('beforeend',copyrightHtml);
            }
        },
    ...
    function initQiscusWidget(userData, windowState) {
        ...
        QiscusSDK.core.getNonce().then(function(res) {
            ...
            function initRoomDone(data) {
                removeClass($('.qcw-cs-container'), 'qcw-cs-container--open');
                $('body').classList.remove('resolved-conversation');
                removeElement($('.qcw-cs-container'));
    ...
    function renderStartNewChat() {
        if (!window.isSessional || $('.start-new-chat-container')) return false;
        var html = '<div class="start-new-chat-container">'
            + '<button>Start New Chat</button>'
            + '</div>';
        $('body .qcw-container').insertAdjacentHTML('afterbegin', html);
    }
        ...
        return new Promise(function (resolve, reject) {
            if(opts.type == 'POST') {
                ...
                if(opts.data) {
                    ...
                    Object.keys(opts.data).forEach(function(d) {
                        formData.append(d, opts.data[d]);
                    })
                }
            }
